...
class ProphetNetCGDecoder (Module):
    ...
    def generate (self, context, audio_emb, video_emb, strategy, beams, max_len):
        ...
        if strategy == 'greedy':
            out_ids = self.model.generate (input_ids=context.view (1, -1), encoder_outputs=ModelOutput (last_hidden_state=enc_emb, hidden_states=None, attentions=None), max_length=max_len)
        elif strategy == 'beam':
            out_ids = self.model.generate (input_ids=context.view (1, -1), encoder_outputs=ModelOutput (last_hidden_state=enc_emb, hidden_states=None, attentions=None), max_length=max_len, num_beams=beams, early_stopping=True)
