...
def update_vocab (vocab, index_to_word, vocab_idx, tokenizer, text):
    tokens = tokenizer (text)
    for tok in tokens:
        if tok.text not in vocab:
            vocab [tok.text] = vocab_idx
            index_to_word [vocab_idx] = tok.text
            vocab_idx += 1
    return vocab_idx

def build_vocab (squad_corpus, vqg_corpus, tokenizer):
    vocab = dict ()
    index_to_word = dict ()
    ...
    for entry in vqg_corpus:
        try:
            vocab_idx = update_vocab (vocab, index_to_word, vocab_idx, tokenizer, entry ['question'])
            vocab_idx = update_vocab (vocab, index_to_word, vocab_idx, tokenizer, entry ['context'])
            vocab_idx = update_vocab (vocab, index_to_word, vocab_idx, tokenizer, entry ['answer'])
        except:
...
if __name__ == '__main__':
    ...
    vocab, index_to_word = build_vocab (preprocessed_text, prep_train + prep_val, tokenizer)
